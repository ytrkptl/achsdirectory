...
class AdminSignIn extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      prevProps.adminRoute !== this.props.adminRoute
    ) {
      this.setState({
        isAdmin: this.props.isAdmin,
        adminRoute: this.props.adminRoute,
  ...
  render() {
    if (this.state.isAdmin === false) {
      ...
      if (this.state.adminRoute === "adminHome") {
        return (
          <h2 style={{ color: "#ff3232" }}>
            Welcome to the admin page.
            <br />
            Use the controls above to <br />
            create, view, edit, or delete (CRUD) <br />
            records from the employee directory.
          </h2>
        );
